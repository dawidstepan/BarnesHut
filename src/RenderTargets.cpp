...
StateOfCircles::StateOfCircles(std::vector<DataPoint> &stateOfDataPoints, float circleRadius = 10.f) {
    // Setup initial list of pointers to circle objects in state
...
void StateOfCircles::update_state_from_bodies(std::vector<Body> &stateOfBodies, bool showVelocity = false) {
...
void StateOfCircles::update_state_from_data_points(std::vector<DataPoint> &stateOfDataPoints, bool showVelocity) {
    ...
    for (int i = 0; i < body_size; i++) {
        ...
        state[i].setPosition(point.x, point.y);
